...
class ZmopClient
{
    ...
    public function execute(Request $request)
    {
        ...
        if ("json" == $this->format) {
            ...
            $bizResponse = $this->getBizResponse($respObject);
            $encrypted = $respObject->encrypted;
            if ($encrypted) {
                $bizResponse = RSAUtil::rsaDecrypt($bizResponse, $this->privateKeyFilePath);
            }
            $bizRespObject = json_decode($bizResponse);
            if (null !== $respObject) {
                $respWellFormed = true;
            }
        }
        ...
        if (isset($respObject->biz_response_sign)) {
            ...
            $verifyResult = RSAUtil::verify($bizResponse, $bizResponseSign, $this->zhiMaPublicKeyFilePath);
            if ($verifyResult == false) {
                throw new ZhimaException("验签失败:".$resp);
            }
        }
     ...
     * @param Request $request 接口请求
     * @return string 用于在浏览器中访问的url
     */
    public function generatePageRedirectInvokeUrl(Request $request)
    {
        $apiParamsQuery = $this->getBizParamStr($request);
     ...
     * 2. 如果验签成功，然后返回解密后的值
     * 3. 如果验签失败，抛出异常
     * @param string $encryptedResponse 加密后的返回值
     * @param string $sign 签名(对未加密的返回值的签名)
     * @return string 解密后的明文
     * @throws RSAValidException
     */
    public function decryptAndVerifySign($encryptedResponse, $sign)
    {
        ...
        if ($success) {
            ...
            throw new RSAValidException("验签失败:".$decryptedResponse);
        }
    }
     ...
     * TODO: 这里逻辑应该有问题
     */
    private function getBizResponse($obj)
    {
    ...
    private function getBizParamStr(Request $request)
    {
    ...
    private function getSystemParams(Request $request)
    {
